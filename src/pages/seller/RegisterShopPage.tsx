...
export default function RegisterShopPage() {
    ...
    const [bannerImage, setBannerImage] = useState<string>("https://plus.unsplash.com/premium_photo-1670738772747-c81429db3725?w=500&auto=format&fit=crop&q=60&ixlib=rb-4.0.3&ixid=M3wxMjA3fDB8MHxmZWF0dXJlZC1waG90b3MtZmVlZHwyfHx8ZW58MHx8fHx8")

    const [shopImage, setShopImage] = useState<string>("https://plus.unsplash.com/premium_photo-1664202525979-80d1da46b34b?w=500&auto=format&fit=crop&q=60&ixlib=rb-4.0.3&ixid=M3wxMjA3fDB8MHxzZWFyY2h8MXx8c2hvcHxlbnwwfHwwfHx8MA%3D%3D")
    ...
    const handleSave = async () => {
        try {
            const response = await ShopService.createShop(name, description, bannerImage, shopImage)
            ...
            <div className="max-w-5xl w-full mx-auto flex flex-col flex-grow ">
                <div className="shadow-all-sides mt-4 flex w-full flex-col gap-6 rounded-md">
                    <div className="relative ">
                        <img className="w-full h-60 rounded-md object-cover" src={bannerImage} alt="" />

                        <label className="group absolute -bottom-12 left-32 rounded-full cursor-pointer">
                            <input onChange={onShopImageChange} className="hidden" type="file" accept="image/*" />
                            <div className="w-fit h-fit relative rounded-full">
                                ...
                                <img className="w-32 h-32 rounded-full object-cover" src={shopImage} alt="" />
                            </div>
                        </label>
                        <label className="absolute right-0 bottom-0 px-8 py-2 rounded-md font-semibold bg-gray-200 m-4 cursor-pointer">
                            <input onChange={onBannerImageChange} className="hidden" type="file" accept="image/*" />
                            <p>Select Photo</p>
                        </label>
                    </div>
                    <div className="p-6 w-full flex flex-col gap-6 mt-6">
                        ...
                        <div className="flex justify-end gap-4">
                            ...
                            <button onClick={handleSave} className="max-w-28 rounded-md bg-primary px-12 py-2 text-sm font-bold text-white">
                                Save
                            </button>
                        </div>
                    </div>
